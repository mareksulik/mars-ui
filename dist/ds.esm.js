...
function createMarsUI(){
  const FALLBACK={ 'gray-100':'#f2f2f2','gray-200':'#ebebeb','gray-600':'#a8a8a8','gray-700':'#8f8f8f','gray-900':'#4d4d4d','gray-1000':'#171717',
    ...
    'lime-800':'#a2a91e','lime-900':'#616f0d','orange-800':'#ff5500','orange-900':'#bd460f','red-700':'#e5484d','red-800':'#da3036','red-900':'#cb2a30','teal-700':'#12a594','teal-900':'#067a6e','purple-700':'#8e4ec6','purple-900':'#7820bc','background-100':'#ffffff' };
  ...
  const status={ ok:()=>token('green-800'), warn3:()=>token('lime-800'), warn10:()=>token('orange-800'), crit:()=>token('red-800'), muted:()=>token('gray-700') };
  ...
  /** Status color by relative deviation — FIXED RULE: met → ok (green), within 3 % → warn3 (lime, still greenish), within 10 % → warn10 (orange), beyond → crit (red).
  ...
  return { version:'1.0.4', token, status, labels:LBL, setLabels, ratioColor, formatters, lineChart, bulletChart, table, badge, yoyChip, paceChip };
}
